export const local = {
    ...
    setItem (key, data) {
        ...
        //存储storage
        window.localStorage.setItem(this.suffix + key,
            JSON.stringify({
                value: data,//真实数据
    ...
    getItem (key) {
        if (key) {
            let localData = JSON.parse(window.localStorage.getItem(this.suffix + key) || '{}');
            if (this.expire > 0) {
                //判断过期与否
                if (Date.now() - localData.expire > this.expire) {//已经过期
                    return null;
                } else {
...
export const session = {
    ...
    */
    setItem (key, data) {
        ...
        //存储storege
        window.sessionStorage.setItem(this.suffix + key,
            JSON.stringify({
                value: data,//真实数据
                expire: this.expire > 0 ? Date.now() : null //过期时间
            })
    ...
    getItem (key) {
        if (key) {
            let localData = JSON.parse(window.sessionStorage.getItem(this.suffix + key) || '{}');
            if (this.expire > 0) {
                //判断过期与否
                if (Date.now() - localData.expire > this.expire) {//已经过期
                    return null;
...
export const sessionStorage = {
    install: function (Vue, options = {}) {
        ...
        Vue.prototype[`$${options.name || 'ss'}`] = session
    }
}
...
export const localStorage = {
    install: function (Vue, options = {}) {
        ...
        Vue.prototype[`$${options.name || 'ls'}`] = local
    }
}